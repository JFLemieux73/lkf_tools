...
## LKF analysis tools
We developed our own set of tools to calculate LKF metrics (e.g. LKF density). The Python code for these metrics is in `lkf_tools/lkf_metrics.py`. There are also Python tools in the directory `lkf_stats_and_plots` to calculate LKF statistics (e.g. mean LKF width) and to plot results.  

Most metrics are the same ones used in :
...
The driver also concatenates the half width data for further analysis. `lkf_stats_and_plots/stats_LKF_width.py` can be used to calculate the mean width and to plot a PDF of the LKF widths.   

### driver_lkf_density.py  

Calculates the fraction of times (between 0 and 1) for the period `SDATE`-`EDATE` a grid cell is crossed by a detected LKF.
